...
## 目前狀態
v0.1.1 四個 PR 全部 merge 進 master（MCP 2025-11-25＋版本觀測＋指紋 v3、release workflow、CI macOS/MSRV、差分 conformance），版本已定版 0.1.1，準備打 v0.1.1-rc1 演練 release workflow。之後開 v0.2.0（WF5 identity → WF4 生命週期 → WF6 benchmark）。
...
## 進行中
- v0.1.1 release 收尾：v0.1.1-rc1 tag 演練（四平台 artifact、attestation 驗證）→ docs/compat.md 手動 client 清單 → 正式 v0.1.1 tag。
- 接續 v0.2.0（計劃檔 ~/.claude/plans/mcpglass-mutable-stardust.md）：WF5 schema v7 identity（lazy re-key）→ WF4 資料生命週期（prune/刪除/錄製模式/DB 權限/診斷包）→ WF6 fail-open 措辭收斂＋benchmark。

## 待辦
- 審查遺留（低優先）：~~指紋 v3 納入 outputSchema~~（0.1.1 已做）；真實 client 的手動相容清單（docs/compat.md，release 前跑）；帶外工具的 Store::open 改真正 read-only 開庫（現況：不記錄流量但對舊版 DB 會做 additive schema 升級，docs/cli.md 已如實揭露）。
