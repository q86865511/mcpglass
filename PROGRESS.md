...
## 目前狀態
v0.1.1 產品化路線執行中（計劃：MCP 2025-11-25＋版本觀測＋指紋 v3、預編譯 binary release、CI macOS/MSRV、差分 conformance）。WF2C（CI 補強＋MSRV 修正 1.86）已完成待 PR；WF1/WF3 實作中。
...
## 進行中
- v0.1.1 產品化（計劃檔 ~/.claude/plans/mcpglass-mutable-stardust.md；git 模式 c：自動 commit＋PR，merge 經使用者確認）：
  - WF1 MCP 2025-11-25＋版本觀測＋指紋 v3（architect agent，worktree 實作中）
  - WF3 預編譯 binary release workflow（implementer agent 已完成，待驗收收線）
  - WF2A 差分 conformance CI（待 WF1）
  - v0.2.0：WF5 schema v7 identity → WF4 資料生命週期 → WF6 fail-open 收斂＋benchmark
