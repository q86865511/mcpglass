...
- [2026-07-16] 🎛️ dashboard GUI 翻新計劃 P2/P3/P5 完成＋README 素材重錄（worktree session-2884c3,計劃全六階段收官）：
  - **P2 Oscilloscope 視覺翻新**：styles.css 拆五檔（tokens/base/layout/components/views）,儀器調色盤（近黑綠底、CH1 磷光綠/CH2 青雙通道色=方向 badge 簽名元素、amber/red 警示 LED）,硬編色歸零（alpha 一律 color-mix）;全域 slim header（wordmark＋LED＋graticule,主題鈕入駐）;儀器刻字＋tabular-nums;響應式 ≤900px drawer＋scrim、≤600px 欄位降級;淺色主題冷灰紙面（對比 text≥13:1）。真實 Chrome 逐視圖像素級目視驗證（本 session 內建瀏覽器面板 frame clock 卡死,transition/截圖皆凍結,以 transition 排除法＋真 Chrome 雙軌驗證）。
  - **P3 UX 元件**：Toast（右下堆疊 4s/aria-live）、ConfirmDialog 取代兩處 window.confirm（focus trap/Esc/Enter）、首載 skeleton、空 detail 儀器待機態、搜尋 250ms debounce、鍵盤 j/k/Enter/Esc//、分頁首尾快跳、auto-refresh 預設開 3s＋visibilitychange 暫停。
  - **P5 新功能前端**：StatsBar EXPORT 下載鈕（<a download> 同源）、sidebar footer Prune 對話框（dry-run 預覽讀數→紅色 Confirm→toast,fingerprints 承諾入框）、Replay 鈕依 health capabilities gating;dev-mock 補 health/prune/export/context/replay 端點。
  - **README 素材**：record-gif 管線改 selector 點擊（免疫版面改動）＋強制 emulate 深色＋localStorage 重置,新增第七幕淺色主題;dashboard-demo.gif（7 幀 2.3MB）與 dashboard-overview.png 以新 UI 重錄;README 功能描述補主題/鍵盤/深連結/dashboard prune/export。
  - 收尾驗證：`cargo build`＋`cargo test --workspace`（19 套件全 ok）＋`clippy --all-targets -D warnings` 零警告、`pnpm build` 零警告;demo.ps1 真流量端到端（wrap/inject/bloat）通過。
- [2026-07-16] 🔌 dashboard 後端補齊（worktree session-2884c3,GUI 翻新計劃 P4）：`POST /api/prune`（older_than_ms/max_size_bytes/dry_run/vacuum,兩條件至少一,vacuum 語意對齊 CLI）＋`GET /api/sessions/{id}/export`（Content-Disposition attachment,**遮罩必經**——CLI export 的 bundle 組裝/mask 下沉為 `dashboard::build_export_bundle`,CLI 反向呼叫,單一遮罩路徑）＋health 回 `{version, capabilities:{replay}}`;storage 新增 `prune_to_max_size`/`preview_max_size`（CLI apply_max_size 下沉,行為零差異）;serve 簽名加 policy、dashboard 子指令加 `--policy`（fail-fast）。主對話重跑 `cargo test --workspace` 全綠＋clippy --all-targets 零警告。
- [2026-07-16] 🧱 dashboard 前端地基（GUI 翻新計劃 P1）：`styles/tokens.css` 設計 token 雙主題（data-theme＋localStorage＋prefers-color-scheme）、`useHashRoute` hash 深連結（`#/s/{id}/{view}?msg=`,session/view/message 單一真相）、`useApi`（AbortController 取代 7 個 seq useRef 競態防護,附 loading/error/retry）;瀏覽器實測主題切換/視圖切換/深連結還原全通過。README 小修（安裝範例 v0.1.1→v0.2.0、CI 說明補 macOS）。
...
## 進行中
（無——GUI 翻新計劃六階段全部完成於 worktree session-2884c3 分支 claude/session-2884c3,待 merge 回 master。）
